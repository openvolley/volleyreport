...
visP_sum <- vsx$meta$players_v %>% dplyr::select(player_id,number, name, starting_position_set1, starting_position_set2, starting_position_set3, starting_position_set4, starting_position_set5, role)  %>% add_row(player_id = "Team total", name = "Team total") %>%
    ...
    left_join(volleyreport::vr_reception(vsx$x, datavolley::visiting_team(vsx$x), file_type = x$file_meta$file_type), by = "player_id", suffix = c(".ser", ".rec") ) %>%
...
Rexc <- !isTRUE(grepl("perana", x$file_meta$file_type)) ## perana have only R#+, not R#
...
visitingSS <- vr_points(vsx$x, datavolley::visiting_team(vsx$x), by = "set") %>%
    ...
    left_join(volleyreport::vr_reception(vsx$x, datavolley::visiting_team(vsx$x), by = "set", file_type = x$file_meta$file_type), by = "set_number", suffix = c(".ser", ".rec") ) %>%
