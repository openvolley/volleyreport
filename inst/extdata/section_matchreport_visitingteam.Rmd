...
visP_sum <- vsx$meta$players_v %>% dplyr::select(player_id,number, name, starting_position_set1, starting_position_set2, starting_position_set3, starting_position_set4, starting_position_set5, role)  %>% add_row(player_id = "Team total", name = "Team total") %>%
    ...
    left_join(volleyreport::vr_reception(vsx$x, datavolley::visiting_team(vsx$x)), by = "player_id", suffix = c(".ser", ".rec") ) %>%
...
visP_sum <- visP_sum %>% dplyr::select(-player_id) %>% dplyr::arrange(.data$number) %>% purrr::discard(~all(is.na(.))) %>% 
    na_if(0)
```
...
kable(visP_sum, format = "html", escape = F, col.names = c("", "", seq_len(nrow(vsx$meta$result)), if (vsx$vote) "Vote", "Tot", "BP", "W-L", "Tot", "Err","Pts", 
    ...
    add_header_above(c(setNames(2,datavolley::visiting_team(vsx$x)), "Set" = nrow(vsx$meta$result), "Points" = 3 + vsx$vote, "Serve" = 3, "Reception" = 4, "Attack" = 5, "Blo" = 1), color = vsx$css$header_colour, background = vsx$css$header_background, bold = TRUE) %>% row_spec(0, bold = TRUE, color = vsx$css$header_colour, background = vsx$css$header_background) %>%
...
visitingSS <- vr_points(vsx$x, datavolley::visiting_team(vsx$x), by = "set") %>%
    ...
    left_join(volleyreport::vr_reception(vsx$x, datavolley::visiting_team(vsx$x), by = "set"), by = "set_number", suffix = c(".ser", ".rec") ) %>%
...
kable(visitingSS, format = kable_format, escape = TRUE, col.names = c("","Ser", "Atk", "Blo","Op.Err", "Tot", "Err","Pts", "Tot", "Err", "Pos%", "(Exc%)", "Tot", "Err", "Blo", "Pts", "Pts%", "Pts")) %>%
    kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = TRUE, font_size = 11)  %>%
