...
section_spacer <- if (!"section_spacing" %in% names(vsx) || isTRUE(vsx$section_spacing == 0)) "" else paste0("<div style=\"height:", vsx$section_spacing, "vh;\"></div>\n")
...
```{r mr_score_ev, warning = FALSE, eval = vsx$style %in% c("ov1"), results = "asis", fig.height = (20 + vsx$plot_summary * 35) / 25.4, fig.width = 210 / 25.4}
...
if (length(vsx$footnotes) > 0) {
    cat(section_spacer)
    cat0("<div style=\"float:right; font-size:", vsx$base_font_size * 8/12, "px;\">", paste(footnotes, collapse = " "), "</div>")
}
```
