...
vsx$x %>% dplyr::select(point_won_by, home_team, home_setter_position, visiting_setter_position, point_id, visiting_team) %>% distinct() %>%
...
cat("<div class=\"row\">")
rhome <- vsx$x %>%
    dplyr::filter((skill %in% c("Serve", "Reception")) | (lead(point == TRUE,2)), team == datavolley::home_team(vsx$x)) %>% 
    dplyr::summarize(Reception = sum(skill == "Reception", na.rm=TRUE),
        'Points SO' = sum(serving_team == datavolley::visiting_team(vsx$x) & 
                          point_won_by == datavolley::home_team(vsx$x) &
                          evaluation_code == "#", na.rm=TRUE)) %>%
...
shome <- vsx$x %>%
    ...
    dplyr::summarize(Serve = sum(skill == "Serve", na.rm=TRUE),
        'Points BP' = sum(serving_team == datavolley::home_team(vsx$x)  & 
                              point_won_by == datavolley::home_team(vsx$x) &
                              evaluation_code == "#", na.rm=TRUE)) %>%
    pivot_longer(cols = 1:2)
...
cat("<div class=\"row\">")
rvis <- vsx$x %>%
    dplyr::filter((skill %in% c("Serve", "Reception")) | (lead(point == TRUE,2)), team == datavolley::visiting_team(vsx$x)) %>% 
    dplyr::summarize(Reception = sum(skill == "Reception", na.rm=TRUE),
        'Points SO' = sum(serving_team == datavolley::home_team(vsx$x) & 
                              point_won_by == datavolley::visiting_team(vsx$x) &
                              evaluation_code == "#", na.rm=TRUE)) %>%
    pivot_longer(cols = 1:2)
...
svis <- vsx$x %>%
    dplyr::filter((skill %in% c("Serve", "Reception")) | (lead(point == TRUE,2)), team == datavolley::visiting_team(vsx$x)) %>% 
    dplyr::summarize(Reception = sum(skill == "Serve", na.rm=TRUE),
        'Points BP' = sum(serving_team == datavolley::visiting_team(vsx$x)  & 
                              point_won_by == datavolley::visiting_team(vsx$x) &
                              evaluation_code == "#", na.rm=TRUE)) %>%
    pivot_longer(cols = 1:2)
